...
def encrypt(plainText, key):
    ...
    def mixColumns(block):
        ...
        newStateMatrix = [
            [
                (
                    multiplyWithinGaloisField(
                        mixColumnConstant[i][0], stateMatrix[0][j]
                    )
                    ^ multiplyWithinGaloisField(
                        mixColumnConstant[i][1], stateMatrix[1][j]
                    )
                    ^ multiplyWithinGaloisField(
                        mixColumnConstant[i][2], stateMatrix[2][j]
                    )
                    ^ multiplyWithinGaloisField(
                        mixColumnConstant[i][3], stateMatrix[3][j]
                    )
                )
                for j in range(4)
            ]
...
def main():
    """
    @desc: Driver method that takes plainText/message and 128bit key \
        and performs AES encryption and decryption on the given message.
    """
    ...
    print(colored(f"deciphered text: {decipheredText}", "magenta"))
